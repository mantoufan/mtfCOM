var mtfBBcode = {
  ...
  listen: function (o) {
    o.find(".mtfBBcode-buy").click(function () {
      var a = $(this).find("sup").html(),
        d = JSON.parse($(this).attr("data"));
      Ply.dialog("confirm", { effect: "3d-sign" }, a).done(function (ui) {
    ...
    if (a.length > 0) {
      ...
      for (var i = 0; i < b.length; i++) {
        var p = o.find(
          '.mtfBB-Menu [href="/' + decodeURIComponent(b[i]) + '"]'
        );
        p.addClass("cur");
  ...
  paste: function (h) {
    var a = [
        new RegExp("\\|" + mtfLang.get("设置"), "g"),
        new RegExp("\\|" + mtfLang.get("数据"), "g"),
        new RegExp("\\[set\\] ", "g"),
        new RegExp("\\[dat\\] ", "g"),
      ],
      s = "";
  ...
  fl: function (o) {
    ...
    o.find("." + c).each(function (a, b) {
      (b = $(b)), (s = $("<input>"));
      s.attr("type", "button"), (t = b.attr("type"));
      if (t === "set") {
  ...
  p: function (o, obj) {
    ...
    if (j) {
      ...
      MTF.T.prompt(t, j).done(function (ui) {
        ...
        if (ui.data.hasOwnProperty("result")) {
          //投票、测试结果数据
