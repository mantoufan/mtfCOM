...
class mtfTpl{
	public function tip($_a){
		...
		(@$_a['bu']?'<meta http-equiv="refresh" content="'.$_a['r'].';url='.@$_a['bu'].'" />':'').
		'<title>'.$_a['t'].'</title>
		<style>
		...
		<body>'.
		(@$_a['a']?'<img class="a" src="'.$_a['a'].'" />':'').
		(@$_a['t']?'<div>'.$_a['t'].'</div>':'').
		(@$_a['d']?'<div class="d">'.$_a['d'].'</div>':'').
		(@$_a['bt']?'<a class="bt" href='.urldecode($_a['bu']).' style="background:'.$_a['bc'].'">'.$_a['bt'].'</a>':'').
		(@$_a['bt1']?'<a class="bt" href='.urldecode($_a['bu1']).' style="background:'.$_a['bc1'].'">'.$_a['bt1'].'</a>':'').
		'</body>
		</html>';
	}
	public function frame($_domain){
		...
		function url_encode($q){
			$b=array();
			$a=explode('/',$q);
			foreach($a as $k=>$v){
				$b[]=substr($v,0,1)==='?'?$v:urlencode($v);
			}
			return implode('/',$b);
		}
		*/
