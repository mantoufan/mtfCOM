...
class mtfFile {
  ...
  public function e2dir($_e) {
    if (in_array($_e, $this->conf['oss']['ext'])) {
      return $this->dir['oss'];
    }
    ...
    if ($_f['t'] === 'image') {
      ...
      while (true) {
        ...
        if (empty($_v['videoSize']) === false || empty($_v['audioSize']) === false) {
          $_filesizekb = 0;
          if (empty($_v['videoSize']) === false) {
            $_filesizekb += $_v['videoSize'];
          }

          if (empty($_v['audioSize']) === false) {
            $_filesizekb += $_v['audioSize'];
          }
          ...
          if ($queue) {
            $this->mtfQueue->urlUpdate($queue, round($_v['time'] / $_f['i']['duration'] * 100, 2) . '%');
          }
        ...
        foreach ($_r as $_k => $_v) {
          ...
          if (empty($_d['video']) === false) {
            ...
            foreach ($_d['video'] as $_v2) {
              ...
              if ($_v2['duration']) {
                $_h .= '<video:duration>' . intval($_v2['duration'] / 1000) . '</video:duration>';
              }
  ...
  public function getContent($_f_p, $_t = 'view', $_arv = array(), $_config = array()) {
    ...
    if ($_f['t'] === 'mtfdat') {
          ...
          if ($_r) {
            ...
            foreach ($_r as $_k => $_v) {
              ...
              if (empty($_key['标题']) === false) {
                ...
                if (empty($_tag) === false) {
                  $__a['k'] = $this->mtfAttr->parseK($_tag);
                }
      ...
      case 'sitemap':
        ...
        $_c = $_h['s'];
        if (empty($_h['bb']) === false) {
          $_ar['bb'] = $_h['bb'];
        }

        if (empty($_h['bb_html']) === false) {
      ...
      case 'list':
        ...
        if (empty($_arv['dm']) === false) {
          $_ar['list']['dm'] = 1;
        } elseif (empty($_a['audio']) === false) {
          $_ar['list']['audio'] = $_a['audio'];
        }

        if (empty($_a['url']) === false) {
          $_ar['list']['url'] = $_a['url'];
        }
  ...
  private function _get_ture_orientation_img($_f_p) {
    ...
    if (function_exists('exif_read_data') === false || in_array($_i['mime'], array('image/png', 'image/jpeg', 'image/pjpeg')) === false) {
      return false;
    }
        ...
        if ($this->hasRight($_uid, $_i, '自己')) {
          ...
          if ($_step === '1') {
            ...
            if (empty($_data['title']) === false) {
              $_tag['标题'] = array($_data['title']);
            }

            if (empty($_data['des']) === false) {
              $_tag['描述'] = array($_data['des']);
            }
  ...
  public function mtfQueuePage($_total, $_per, $_page) {
    ...
    if ($_per) {
      ...
      if ($_p > 1) {
        $_r[1] = array('p' => 1, 's' => 1);
        $_start = $_page > 1 ? $_page - 1 : 1;
        $_end = $_page + 1 < $_p ? $_page + 1 : $_p;
  ...
  public function mtfQueueList($_dat = array()) {
        ...
        if (empty($_tdk) && empty($_dat['dm']) && empty($_dat['sub']) && empty($_dat['mom'])) {
          ...
          if ($_tag_i) { //标签
            ...
            if ($__r) {
              if (empty($_key)) {
                $_key = array();
              } elseif (isset($_key['描述']) && empty($_key['描述'][0]) === false) {
                $_des = $_key['描述'][0];
              }
    ...
    if ($_tpl === 'data') {
      ...
      if (empty($_weal) === false) {
        $_rr['weal'] = $_weal;
      }

      return $_rr;
    } else {
      if (empty($_relate) === false) {
      ...
      foreach ($_data as $_f_p) {
        ...
        if ($_f['t'] === 'image') {
          ...
          foreach ($this->conf['convert'][$_f['t']] as $__v) {
            ...
            if (empty($__v['w']) === false) {
              $_d['convert']['w'] = $__v['w'];
            }

            if (empty($__v['force']) === false) {
